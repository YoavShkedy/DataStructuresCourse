...
class AVLTreeListTester2022(unittest.TestCase):
    """ We made the is_Tree_AVL test easier in order to ignore mistakes made by wrong
        implementation of getLeft() and getRight() methods.
    ...
    def is_Tree_AVL(self, tree: AVLTreeList, checkFirstAndLast=True, split=False):
        root = tree.getRoot()
        if split:
            check(tree)
            print(tree)
        if root is not None:
            if checkFirstAndLast:
                ...
                self.assertEqual(leftMostNode.getValue(), tree.first(),
                                 "first() doesn't return the value of the first node")
                ...
                self.assertEqual(rightMostNode.getValue(), tree.last(),
                                 "last() doesn't return the value of the last node")
    ...
    def is_node_valid_rec(self, node: AVLNode):
        if node is not None and node.isRealNode():
            if node.getRight() is not None and node.getRight().isRealNode():
                self.assertEqual(node.getRight().getParent(), node,
                                 "Right son of node with value " + str(node.getValue()) +
                                 " doesn't point to it's parent")
            if node.getLeft() is not None and node.getLeft().isRealNode():
                self.assertEqual(node.getLeft().getParent(), node,
                                 "Left son of node with value " + str(node.getValue()) +
                                 " doesn't point to it's parent")
    ...
    def test_basic_avl_node_get(self):
        ...
        # self.assertIsNone(root.getParent(), "FAIL - root's parent should be None") removed this test, 
        # could be VirtualNode (implementation decision) 
    ...
    def test_empty_after_deletion(self):
        ...
        for i in range(19):
            ...
            self.assertFalse(T.empty(),
                             "FAIL - tree.empty() on a non-empty tree should return False, iteration " + str(i))
        T.delete(0)
    ...
    def test_delete_operations_simple_2(self):
        ...
        self.tree.insert(0, "8")
        self.tree.insert(1, "10")
        self.tree.insert(0, "6")
        self.tree.insert(0, "4")
        self.tree.insert(1, "5")
    ...
    def test_first_and_last_with_length(self):
        for i in range(10):
            ...
            self.assertEqual(self.tree.last(), str(i * 10 + 9),
                             "FAIL - tree.last() should return '{}'".format(i * 10 + 9))
        self.add_points(2)

    def test_list_to_array_empty(self):
        self.assertEqual(self.tree.listToArray(), [], "FAIL - listToArray() should return [] for an empty tree")
        self.tree.insert(0, "1")
        self.assertNotEqual(self.tree.listToArray(), [],
                            "FAIL - listToArray() should not return [] for a non-empty tree")
        self.add_points(1)
    ...
    def test_list_to_array_identical_vals(self):
        ...
        self.assertEqual(lst, T.listToArray(),
                         "FAIL - listToArray() is not consistent with the insertion order provided")
        self.add_points(1)
    ...
    def test_list_to_array(self):
        ...
        self.assertEqual(origSentence, treeSentence,
                         "FAIL - the listToArray method creates the sentence:\n'{}'".format(treeSentence))
    ...
    def test_list_to_array_random(self):
        ...
        for i in range(10):
            for j in range(10):
                ...
                length += 1
            AVLList = self.tree.listToArray()
            self.assertEqual(python_list, AVLList,
    ...
    def test_length_after_insert(self):
        ...
        for i in range(10):
            ...
            length += 1
            self.assertEqual(length, self.tree.length(),
    ...
    def verify_split(self, tree: AVLTreeList, i: int, grade, edge_case=False):
        ...
        self.assertEqual(self.in_order(lower_values), original_lower_values,
                         "FAIL - Problem with split({}), left tree".format(i))
        self.assertEqual(self.in_order(higher_values), original_higher_values,
                         "FAIL - Problem with split({}), right tree".format(i))
        self.add_points(grade / 2, False)
        if not edge_case:
            self.is_Tree_AVL(lower_values, True, True)
            self.is_Tree_AVL(higher_values, True, True)
            self.add_points(grade / 2, False)
        else:
            if i == 0:
                self.assertTrue(lower_values.empty(), "FAIL - 'left' should be empty after split on index 0")
            else:
                self.assertTrue(higher_values.empty(), "FAIL - 'right' should be empty after split on last index")
            self.add_points(grade / 2, False)
    ...
    def verify_concat(self, tree_1: AVLTreeList, tree_2: AVLTreeList):
        ...
        self.assertEqual(returned_value, height_diff, "FAIL - concat returned {} when it should have returned {}" \
    ...
    def calc_height_diff(self, tree_1: AVLTreeList, tree_2: AVLTreeList):
        if tree_1.empty() and tree_2.empty():
            ...
            return tree_2.getRoot().getHeight() + 1
        elif tree_2.empty():
            return tree_1.getRoot().getHeight() + 1
    ...
    def test_concat_simple(self):
        ...
        first_part = sentence[0:16]
        second_part = sentence[16:]
    ...
    def test_concat_empty_tree(self):
        ...
        before_concat_in_order = self.in_order(T)
        before_concat_length = T.length()
        before_concat_first = T.first()
        before_concat_last = T.last()
        height_diff = self.calc_height_diff(T, empty_tree)
        concat_ret_value = T.concat(empty_tree)
        after_concat_in_order = self.in_order(T)
        after_concat_length = T.length()
        after_concat_first = T.first()
        after_concat_last = T.last()
    ...
    def test_concat_to_empty_tree(self):
        ...
        after_concat_in_order = self.in_order(empty_tree)
        after_concat_length = empty_tree.length()
        after_concat_first = empty_tree.first()
        after_concat_last = empty_tree.last()
        self.assertEqual(before_concat_in_order, after_concat_in_order, "FAIL - in order traversal should be equal " \
                                                                        "before and after concatenating to an empty "
                                                                        "tree")
        self.assertEqual(before_concat_length, after_concat_length, "FAIL - length should be equal before and after " \
    ...
    def test_concat_many_lists(self):
        list_of_trees = [self.create_tree([str(i) for i in range(100 * j, 100 * (j + 1))], random_order=True) for j in
                         range(10)]
        ...
        for tree in list_of_trees[1:]:
            ...
            self.assertEqual(x := final_tree.concat(tree), height_diff,
                             "FAIL - concat returned {} when it should have return {}" \
    ...
    def test_search_after_delete(self):
        ...
        T = self.create_tree([str(i) for i in range(N)], random_order=True)
        for i in range(N):
            self.assertNotEqual(T.search(str(i)), -1,
                                "FAIL - search should return -1 iff str({}) is not in the tree".format(i))
            T.delete(T.search(str(i)))
            self.assertEqual(T.search(str(i)), -1,
                             "FAIL - search should return -1 iff str({}) is not in the tree".format(i))
        self.add_points(2)
